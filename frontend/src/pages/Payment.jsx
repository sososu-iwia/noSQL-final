  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await paymentsAPI.payBooking({
        bookingId,
        cardNumber,
        expMonth: Number(formData.expMonth),
        expYear: Number(formData.expYear),
        cvv: String(formData.cvv),
      });

      if (response.data.success) {
        alert(
          `Оплата успешна! PNR: ${response.data.pnr}\nE-ticket отправлен на вашу почту.`,
        );
        navigate(`/bookings/${bookingId}`);
      }
    } catch (err) {
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-4">
              <div className="flex items-start">
                ...
                <div className="text-sm text-blue-800">
                  <strong>Безопасность:</strong> Данные карты обрабатываются
                  безопасно. Последние 4 цифры карты будут сохранены для
